...
use crate::ast::{expression::{Attr, Call, Disambiguation, Expr, Functor, Lookup, MethodCall, Operator, Variant}, statement::{Fn, FnArg, Let, Stmt, Type}, ty::{SpecializedTy, Ty}};
...
fn parse_type<'s>(input: Pair<'s, Rule>) -> Result<Ty<'s>, ()> {
    ...
    match first.as_rule() {
        ...
        _ => {
            ...
            let generic_args = inner.next().map(|p| {
                p.into_inner().map(|p| parse_type(p)).collect::<Result<Vec<_>, _>>()
...
fn parse_expr2<'s>(input: Pair<'s, Rule>) -> Result<Expr<'s>, ()> {
    ...
    while let Some(inner) = inners.next() {
        match inner.as_rule() {
            ...
            Rule::call => {
                let arg_pairs = inner.into_inner().next().unwrap().into_inner().next().map(|p| p.into_inner());
                let mut args = Vec::new();
...
static BIN_PRATT: LazyLock<PrattParser<Rule>> = LazyLock::new(||{
    ...
    .op(Op::infix(Rule::BINARY_EQ, Assoc::Left) | Op::infix(Rule::BINARY_NE, Assoc::Left)
        | Op::infix(Rule::BINARY_GT, Assoc::Left) | Op::infix(Rule::BINARY_LT, Assoc::Left)
        | Op::infix(Rule::BINARY_GE, Assoc::Left) | Op::infix(Rule::BINARY_LE, Assoc::Left))
    .op(Op::infix(Rule::BINARY_BIT_AND, Assoc::Left) | Op::infix(Rule::BINARY_BIT_OR, Assoc::Left)
        | Op::infix(Rule::BINARY_BIT_XOR, Assoc::Left))
    .op(Op::infix(Rule::BINARY_ADD, Assoc::Left) | Op::infix(Rule::BINARY_SUB, Assoc::Left))
    .op(Op::infix(Rule::BINARY_MUL, Assoc::Left) | Op::infix(Rule::BINARY_DIV, Assoc::Left)
...
fn parse_expr<'s>(input: Pair<'s, Rule>) -> Result<Expr<'s>, ()>{
    ...
    (*BIN_PRATT).map_primary(parse_expr1)
...
fn parse_statement<'s>(input: Pair<'s, Rule>) -> Result<Stmt<'s>, ()> {
    ...
    match first.as_rule() {
         ...
         Rule::function => {
            ...
            let [raw_name, gen_params, params, ret_ty, body] = [inners.next().unwrap(), inners.next().unwrap(), inners.next().unwrap(), inners.next().unwrap(), inners.next().unwrap()];
            let name = raw_name.as_str();
            let gen_params = gen_params.into_inner().map(|p| p.as_str().into()).collect();
            let params = params.into_inner().next().map(|params| {
                params.into_inner().map(|p| {
...
pub(crate) fn parse_raw_statement<'s>(input: &'s str) -> Result<Stmt<'s>, pest::error::Error<Rule>> {
    return parse_statement(DinoParse::parse(Rule::declaration, input)?.next().unwrap()).map_err(|_| todo!());
}

pub(crate) fn parse_raw_statements<'s>(input: &'s str) -> Result<Vec<Stmt<'s>>, pest::error::Error<Rule>> {
    return DinoParse::parse(Rule::header, input)?.next().unwrap().into_inner().next().unwrap().into_inner()
            .map(parse_statement).collect::<Result<Vec<_>,_>>().map_err(|_e| todo!());
}