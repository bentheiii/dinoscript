            ...
            Ty::Fn(assign_fn) => {
                if let Ty::Fn(inp_fn) = input_type.as_ref() {
                    ...
                    assign_fn
                        .args
                        .iter()
                        .zip(inp_fn.args.iter())
                        .map(|(a, i)| self.assign(a, i))
        ...
        (Ty::Specialized(a_spec), Ty::Specialized(b_spec)) => {
            ...
            if Arc::ptr_eq(a_templ, b_templ) {
                ...
                let args = a_spec
                    .args
                    .iter()
                    .zip(b_spec.args.iter())
                    .map(|(a, b)| combine_types(a, b))
                    .collect::<Result<Vec<_>, _>>()?;
                Ok(Arc::new(Ty::Specialized(Specialized {
                    template: a_templ.clone(),
                    args,
                })))
            } else {
                Err(())
